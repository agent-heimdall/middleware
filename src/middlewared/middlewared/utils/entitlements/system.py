...
def get_facts() -> EntitlementFacts:
    ...
    Deliberately not cached. A license can be uploaded or removed under a running
    middlewared, and an answer that survives that is a bug -- which is exactly what
    ``SystemService.PRODUCT_TYPE`` was. ``get_hardware_class()`` is cheap after the
    first call in a process: ``get_hardware_info()`` is ``@cache``d, so only that
    first call forks dmidecode and walks sysfs.
    """
