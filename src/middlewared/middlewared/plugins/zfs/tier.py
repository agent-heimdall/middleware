...
from middlewared.utils.zfs.guard import deny_protected_path
...
def _raise_client_error(e: RewriteClientException, field: str) -> None:
    ...
    if e.code == JSONRPCErrorCode.DISABLED:
...
def get_dataset_tier_info_cached(
    ...
    if pool_name not in pool_special_cache:
        try:
            pool = hdl.open_pool()
            props = pool.get_properties(properties={truenas_pylibzfs.ZPOOLProperty.CLASS_SPECIAL_SIZE})
    ...
    try:
        ...
        tier_type = "PERFORMANCE" if rs_value > 0 and ssb_value >= rs_value else "REGULAR"
...
class ZfsTierRewriteJobQueryEventSource(TypedEventSource[ZfsTierRewriteJobQueryEventSourceArgs]):
    """
...
class ZfsTierService(GenericConfigService[ZfsTierEntry]):
    ...
    async def rewrite_job_create(self, data: dict[str, typing.Any]) -> dict[str, typing.Any]:
        ...
        if not config.enabled:
            raise ValidationError(field, "ZFS tiering is globally disabled", errno.EINVAL)
    ...
    async def _validate_dataset_writable(self, dataset_name: str, field: str) -> None:
        ...
        if props["mounted"]["raw"] == "no":
            raise ValidationError(field, f"{dataset_name!r}: dataset is not mounted", errno.EINVAL)
    ...
    def _validate_tier_space(
        ...
        if target_tier == "PERFORMANCE":
            ...
            if usable > 0:
                projected_pct = (space_info["class_normal_used"] + dataset_used) / usable * 100
                if projected_pct > 80:
                    raise ValidationError(
                        field,
    ...
    async def dataset_set_tier(self, data: dict[str, typing.Any]) -> dict[str, typing.Any]:
        ...
        deny_protected_path("zfs_tier_dataset_set_tier.dataset_name", data["dataset_name"])
        ...
        config = await self.config()
        if not config.enabled:
            raise ValidationError("zfs_tier_dataset_set_tier", "ZFS tiering is globally disabled", errno.EINVAL)
        ...
        if tier_job and tier_job["status"] in (
            ...
            raise ValidationError(
                field,
                f"{dataset_name!r}: a tier migration job is already in progress (status: {tier_job['status']})",
                errno.EBUSY,
            )
        ...
        if move_existing_data:
            job_entry = await self.call2(self.rewrite_job_create, {"dataset_name": dataset_name})

        new_tier_info = (await self.call2(self.bulk_get_tier_info, [dataset_name]))[dataset_name]
    ...
    def bulk_tier_info_with_sentinel(self, tls: typing.Any, dataset_names: list[str]) -> dict[str, typing.Any]:
        """
    ...
    @private
    @pass_thread_local_storage
    def bulk_get_tier_info(self, tls: typing.Any, dataset_names: list[str]) -> dict[str, typing.Any]:
        """
