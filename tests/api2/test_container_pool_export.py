...
def test_cascade_export_keeps_container_records(image):
    ...
    The rootfs datasets go with the pool and are still there, so removing the records would
    orphan live storage with no way to get it back -- the container's definition and its idmap
    slice exist nowhere else.
    """
    with another_pool({"name": POOL_NAME}) as pool:
        with container(image, options={"pool": POOL_NAME}, start=True) as c:
            call(
                "pool.export", pool["id"], {"cascade": True, "destroy": False}, job=True
            )

            assert container_exists(c["id"]), (
                "cascaded export destroyed the container's records"
            )

            call(
                "pool.import_pool", {"guid": pool["guid"], "name": POOL_NAME}, job=True
            )

            # The record still points at storage that is really there, so the container runs again
            assert call("container.get_instance", c["id"])["dataset"] == c["dataset"]
            call("container.start", c["id"])
            assert (
                call("container.get_instance", c["id"])["status"]["state"] == "RUNNING"
            )
...
def test_cascade_destroy_removes_container_records(image):
    ...
    Both a running and a stopped container are checked: the delegate only ever saw active
    containers, which made the old cleanup drop the records of running containers and keep those
    of stopped ones.
    """
    with another_pool({"name": POOL_NAME}) as pool:
        with (
            container(
                image, options={"pool": POOL_NAME}, name="running", start=True
            ) as running,
            container(image, options={"pool": POOL_NAME}, name="stopped") as stopped,
        ):
            call(
                "pool.export", pool["id"], {"cascade": True, "destroy": True}, job=True
            )

            assert not container_exists(running["id"])
            assert not container_exists(stopped["id"]), (
                "records of stopped containers survived a cascaded destroy"
            )


def test_cascade_destroy_of_offline_pool_keeps_container_records(image):
    """Asking to destroy an OFFLINE pool does not destroy anything, so the records must stay.

    An OFFLINE pool is not imported, so `pool.export` skips both the zpool destroy and the disk
    wipe and the container's rootfs survives untouched. Keying the cleanup off the requested
    `destroy` option instead of what actually happened would orphan that storage -- the exact
    ...
    with another_pool({"name": POOL_NAME}) as pool:
        with container(image, options={"pool": POOL_NAME}) as c:
            try:
                ...
                assert container_exists(c["id"]), (
                    "records were dropped for a pool that was never actually destroyed"
                )
            finally:
                # Bring the pool back so the container and the pool can be torn down normally.
                if any(
                    p["guid"] == pool["guid"]
                    for p in call("pool.import_find", job=True)
                ):
                    call(
                        "pool.import_pool",
...
def test_dataset_delete_of_bind_mount_source_keeps_container(image):
    """Deleting a dataset a container bind-mounts costs it the mount, not its whole definition.

    `pool.dataset.delete` has no cascade flag, so it can only ever take the conservative action.
    """
        ...
        with dataset("media", pool=POOL_NAME) as media:
            with container(image, options={"pool": POOL_NAME}) as c:
                with filesystem_device(c["id"], f"/mnt/{media}", "/data"):
                    call("container.start", c["id"])
                    assert (
                        call("container.get_instance", c["id"])["status"]["state"]
                        == "RUNNING"
                    )
                    ...
                    assert container_exists(c["id"]), (
                        "deleting a bind-mount source deleted the container"
                    )
                    assert (
                        call("container.get_instance", c["id"])["status"]["state"]
                        == "STOPPED"
...
def test_container_storage_remapped_after_pool_rename(image):
    ...
    The dataset is always `<pool>/.truenas_containers/containers/<name>`, so the new location is
    derived rather than guessed, and the container is usable again after the import.
    """
